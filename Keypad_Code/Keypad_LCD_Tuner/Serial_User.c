...
#if 0
    if (EEPROM_Info.rom_size > MEMORY_16K) /* over 32Kbit are 2bytes address */
    {
        SIOr = (uint8_t)(g_PARAA.eepromaddr >> 8U); /* send high address */
    }
    else
    {
       // g_comstatus++;                      /* change status to last addr. */
        SIOr = (uint8_t)(g_PARAA.eepromaddr & 0xffU); /* send one address */
    }
#endif
	SIOr = 0x00;
}
...
{
    g_comstatus++;
    SIOr = /*(uint8_t)(g_PARAA.eepromaddr & 0xffU)*/0x23;   /* send last address */
}
